...
module.exports = {
  async up(queryInterface, Sequelize) {
    console.log('🚀 Creating complete database schema with comprehensive CASCADE delete system...');
    ...
    try {
      ...
      await queryInterface.createTable('athletes', {
        ...
        type: {
          type: Sequelize.ENUM('Athlete', 'Coach', 'Staff'),
          allowNull: false
        },
        gender: {
        ...
        sweep_scull: {
          type: Sequelize.ENUM('Sweep', 'Scull', 'Both'),
          allowNull: true
        },
        port_starboard: {
          type: Sequelize.ENUM('Port', 'Starboard', 'Both'),
          allowNull: true
        },
        bow_in_dark: {
      ...
      // 5. Create Team Memberships table
      ...
      // 7. Create Practice Sessions table
      ...
      // 10. Create Seat Assignments table
      ...
      await queryInterface.createTable('ladder_positions', {
        ...
        streak_type: {
          type: Sequelize.ENUM('none', 'win', 'loss'),
          allowNull: false,
          defaultValue: 'none'
        },
        streak_count: {
      ...
      await queryInterface.createTable('ladder_progressions', {
        ...
        reason: {
          type: Sequelize.ENUM('win', 'loss', 'draw', 'forfeit', 'adjustment'),
          allowNull: false
        },
        match_id: {
          type: Sequelize.UUID,
      ...
      await queryInterface.addIndex('mailing_lists', ['team_id'], { name: 'idx_mailing_lists_team_id', transaction });
      ...
      await queryInterface.addIndex('gauntlet_matches', ['gauntlet_id'], { name: 'idx_gauntlet_matches_gauntlet', transaction });
      await queryInterface.addIndex('gauntlet_matches', ['match_date'], { name: 'idx_gauntlet_matches_date', transaction });
      
      // Gauntlet lineups indexes
      await queryInterface.addIndex('gauntlet_lineups', ['gauntlet_id'], { name: 'idx_gauntlet_lineups_gauntlet', transaction });
      await queryInterface.addIndex('gauntlet_lineups', ['match_id'], { name: 'idx_gauntlet_lineups_match', transaction });
      await queryInterface.addIndex('gauntlet_lineups', ['boat_id'], { name: 'idx_gauntlet_lineups_boat', transaction });
      
      // Gauntlet seat assignments indexes
      await queryInterface.addIndex('gauntlet_seat_assignments', ['gauntlet_lineup_id'], { name: 'idx_gauntlet_seat_assignments_lineup', transaction });
      await queryInterface.addIndex('gauntlet_seat_assignments', ['athlete_id'], { name: 'idx_gauntlet_seat_assignments_athlete', transaction });
      
      // Ladders indexes
      await queryInterface.addIndex('ladders', ['gauntlet_id'], { name: 'idx_ladders_gauntlet', transaction });
      
      // Ladder positions indexes
      await queryInterface.addIndex('ladder_positions', ['ladder_id'], { name: 'idx_ladder_positions_ladder', transaction });
      await queryInterface.addIndex('ladder_positions', ['athlete_id'], { name: 'idx_ladder_positions_athlete', transaction });
      await queryInterface.addIndex('ladder_positions', ['position'], { name: 'idx_ladder_positions_position', transaction });
      
      // Ladder progressions indexes
      await queryInterface.addIndex('ladder_progressions', ['ladder_id'], { name: 'idx_ladder_progressions_ladder', transaction });
      await queryInterface.addIndex('ladder_progressions', ['athlete_id'], { name: 'idx_ladder_progressions_athlete', transaction });
      await queryInterface.addIndex('ladder_progressions', ['match_id'], { name: 'idx_ladder_progressions_match', transaction });
      await queryInterface.addIndex('ladder_progressions', ['date'], { name: 'idx_ladder_progressions_date', transaction });

      await transaction.commit();

      console.log('✅ Complete database schema created successfully!');
      console.log('');
      console.log('🗃️  Database includes 22 tables:');
      console.log('');
      console.log('📊 Core Boathouse Management (15 tables):');
      console.log('  ├── usra_categories');
      ...
      console.log('');
      console.log('🗑️  Comprehensive CASCADE Delete System:');
      console.log('  ✅ Single point of control for all major entities');
      console.log('  ✅ Complete data integrity across all relationships');
      console.log('  ✅ No orphaned records possible');
      console.log('  ✅ Clean, atomic deletion operations');
      console.log('');
      console.log('🎯 Key Features:');
      console.log('  ✅ Teams can have zero or one mailing list');
      console.log('  ✅ Simplified gauntlet system with single ladder per gauntlet');
      console.log('  ✅ Complete practice session and lineup management');
