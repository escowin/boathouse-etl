...
async function debugGoogleSheets() {
  ...
  try {
    ...
    console.log(`🔍 Response range: ${response.data.range}`);
    console.log(`🔍 Response majorDimension: ${response.data.majorDimension}`);
    console.log(`🔍 First row length: ${rows[0]?.length || 0}`);
